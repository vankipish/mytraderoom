...
class cmb_Meta_Box {
			...
			if ( ! is_admin() ) {
			   ...
			   wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), self::CMB_VERSION );
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'rendition' ),
					'defaultString' => __( 'Default', 'rendition' ),
					'pick'          => __( 'Select Color', 'rendition' ),
					'current'       => __( 'Current Color', 'rendition' ),
				) );
			}
		} else {
	 ...
	 * and empty string if no value has been set on the options page.
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'rendition' ) );
