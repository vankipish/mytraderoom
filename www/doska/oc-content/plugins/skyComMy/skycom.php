...
$com_id = globper('com_id');
	$aSkyCom = array('com_kto'=> '',
					 'com_email'=> '',
					 'com_text'=>''
					);
	$skyComNI -> update($aSkyCom,array('com_id' =>$com_id));
}
...
{	$com_id = globper('com_id');
	...
	$aSkyCom = array('com_text'=> $com_text);
	$skyComNI -> update($aSkyCom,array('com_id' =>$com_id));
}
...
//подсчет комментариев
	$skyComNI ->dao->select('COUNT(*)');
	$skyComNI->dao->from('skycom');
	$conditions = array('com_kgol'  => $com);
	$skyComNI->dao->where($conditions);
	$skybase = $skyComNI->dao->get();
...
return $com_sl; // возвращаем ид комментария для вывода комментов к нему
	...
	$conditions = array('com_kgol'  => $com,
						'com_papa' => '0');
	$skyComNI->dao->where($conditions);
	$skybase1 = $skyComNI->dao->get();

	$temp = db2_fetch_array($skybase1);
	$posts = $temp[0];
	$total = (($posts - 1) / $num) + 1;
	$total = intval($total);
	$page = intval($page);
...
{	$skyComNI = skyCom::newInstance();
	if ($com_papa==0) {
		...
		$conditions = array('com_kgol'  => $com,
							'com_papa' => $com_papa);
		$skyComNI->dao->where($conditions);
		$skyComNI->dao->orderBy("com_id",'DESC');
		$skybasecom = $skyComNI->dao->get();
	} else {
		$skyComNI ->dao->select('*');
		$skyComNI->dao->from('skycom');
		$conditions = array('com_kgol'  => $com,
...
function pokaz_com($com,$com_width,$s,$com_dlina,$start,$num,$prava) { //к чему комментарий, ширина комментария, id к чему ответ
	...
	$skyrowcom = db2_fetch_array($skybasecom);
	...
	while($skyrowcom = db2_fetch_array($skybasecom));
	if ($com_width>540) { $com_width = $com_width+20; }
	}
}
//показываем
