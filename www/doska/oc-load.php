...
if( !file_exists(ABS_PATH . 'config.php') ) {
    ...
    $title   = 'Ошибка Osclass';
    $message = 'Кажется отсутствует файл <code>config.php</code>. Osclass еще не установлен. <a href="http://os-class.ru/frm">Нужна помощь?</a></p>';
...
function osc_show_maintenance() {
    if(defined('__OSC_MAINTENANCE__')) { ?>
        <div id="maintenance" name="maintenance">
             <?php _e("Сайт в настоящее время проходит техническое обслуживание"); ?>
        </div>
        <style>
            #maintenance {
                position: static;
...
// register scripts
osc_register_script('jquery', osc_assets_url('js/jquery-1.12.3.min.js'));
